...
archivo_origen="/var/lib/samba/Compartida/Simulador/soluciones_mejora.txt"

# Ruta de destino donde se copiará el archivo
carpeta_destino="/var/lib/samba/Compartida/Ingeniería/"

# Nombre del archivo destino con timestamp
timestamp=$(date +%Y%m%d%H%M%S)
archivo_destino="${carpeta_destino}soluciones_mejora_${timestamp}.txt"
...
if [ -e "$archivo_origen" ]; then
    ...
    if [ $? -eq 0 ]; then
        echo "Archivo enviado a la carpeta Ingeniería: $archivo_destino"
    else
